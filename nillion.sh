...
echo ""
echo "Follow: https://x.com/GolikAgata"
read -p "Have you followed @GolikAgata on X? (y/Y to proceed): " FOLLOWED

if [[ ! "$FOLLOWED" =~ ^[yY]$ ]]; then
    echo ""
    echo "Please follow @GolikAgata on X before proceeding."
    exit 1
fi
...
mkdir -p nillion/accuser4 && \
sudo docker run -v "$(pwd)/nillion/accuser4:/var/tmp" nillion/retailtoken-accuser4:latest initialise

SECRET_FILE=~/nillion/accuser4/credentials.json
